...
class App extends Component {
  constructor(props){
    super(props)
    this.stepsList = [{style: {width: "420px"}, flex: "ff fleft", padding:"60px", step: "First step", img: "./src/assets/01-il (1).svg", description: "We determined that you're our target audience", footer: "Yes, we paid about $1 to get you here", num:"01", name:"block", side:"left"},
  ...
  render() {
    return (
      ...
      <div>
        ...
        <Header/>
        {
             this.stepsList.map(({ style, flex, padding, step, img, description, footer, num, name, side}) => {
               return <First style={style} flex={flex} padding={padding} step={step} img={img} description={description} footer={footer} num={num} name={name} side={side}/>
             }
             )
       }
       <Gallery />
